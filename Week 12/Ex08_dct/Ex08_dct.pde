/*
   ...
   Exercise 8: Experiment with DCT compression
...
void dct8x8(int[][] f, int[][] A, int[][] B) 
{
  ...
  for (int i = 0; i < N; i++) 
  { //Now do 1d dct on each row of B
...
PImage diff(PImage img1, PImage img2) 
{
  ...
  for (int y = 0; y < img1.height; y++)
  {
    for (int x = 0; x < img1.width; x++)
    {
      ...
      max = (abs(red(c2) - red(c1)));
      newMax = max(newMax, max);
      diffImg.set(x, y, color(max, max, max));
    }
  }
  
  scaleFactor = 255.0 / newMax; // Scale diff image to full dynamic range
  
  for (int y = 0; y < diffImg.height; y++)
  {
    for (int x = 0; x < diffImg.width; x++)
    {
      color c = diffImg.get(x, y);
      float scaledValue = red(c) * scaleFactor; 
      diffImg.set(x, y, color(scaledValue, scaledValue, scaledValue));
    }
  } 
  
  println(newMax, scaleFactor);
  return diffImg;
}
